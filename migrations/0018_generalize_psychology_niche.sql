-- ─────────────────────────────────────────────────────────────────────────────
-- عمومی‌سازی نیچ روانشناسی — قابل استفاده برای هر تخصص (نه فقط روانشناسی کودک)
--
-- طبق تصمیم صریح صاحب پروژه: داده‌های فعلی همه تستی‌اند، پس این یک rename
-- واقعی است (نه additive-فقط) — بدون نگرانی از‌دست‌رفتن داده‌ی واقعی.
--
-- مدل قبلی فرض می‌کرد هر پرونده = «کودک» + «پدر» + «مادر». مدل تازه:
--   • client_name  = مراجع (هرکسی، هر سنی)
--   • contact_phone/contact_name = تماس اصلی (برای ورود/OTP) — می‌تواند خود
--     مراجع باشد (بزرگسال) یا یک نفر دیگر (والدین کودک، همراه سالمند، و...)
--   • contact2_name/contact2_phone = تماس دوم اختیاری (مثلا والدین دیگر،
--     همسر، یا هر همراه دیگری) — کاملا اختیاری، اگر لازم نبود خالی می‌ماند
--   • psy_resource_profiles.companion_label = برچسب همین تماس دوم، به‌انتخاب
--     خود هر متخصص («والدین»/«همسر»/«همراه»/خالی=اصلا استفاده نمی‌کند)
--
-- جلسات/پروتکل هم از «کودک/والدین» به «اصلی/دومی» (primary/secondary) تغییر کرد
-- — همان برچسب companion_label روی «دومی» نمایش داده می‌شود.
-- ─────────────────────────────────────────────────────────────────────────────

-- psy_cases: هویت پرونده عمومی شد
...
-- ستون phone خام کپی بی‌مصرف contact_phone بود؛ parent_name هم مشتق‌شده و هیچ‌جا نمایش داده نمی‌شد
...
-- psy_resource_profiles: برچسب قابل‌تنظیم «تماس دوم» — خالی یعنی این متخصص
-- اصلا از این مفهوم استفاده نمی‌کند (مثلا درمان فردی بزرگسال)
