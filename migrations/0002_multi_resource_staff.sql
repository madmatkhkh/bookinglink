-- ─────────────────────────────────────────────────────────────────────────────
-- Migration 0002: چندکارمندی/چندمنبعی برای نیچ روانشناسی + ورود کارمند
--
-- برخلاف رویه‌ی معمول پروژه (drop-all + rerun)، این فایل عمدا additive است:
-- روی یک دیتابیس زنده با پرونده‌های واقعی هم امن اجرا می‌شود، چون:
--   • هیچ جدولی drop نمی‌شود.
--   • resource_id ابتدا nullable اضافه می‌شود، بعد backfill، بعد not null.
--   • برای هر tenant، همه‌ی رکوردهای قدیمی به همان «منبع پیش‌فرض»ی که در زمان
--     اولین لاگین دکتر ساخته شده بود گره می‌خورند — یعنی رفتار tenantهای
--     تک‌دکتر فعلی هیچ تغییری نمی‌کند.
--
-- اجرا: کل این فایل را یک‌جا در SQL Editor سوپابیس اجرا کنید.
-- ─────────────────────────────────────────────────────────────────────────────

-- ── 1) resources: امکان ورود کارمند ────────────────────────────────────────
...
-- ── 2) resource_id روی جدول‌های تخصصی روانشناسی ─────────────────────────────
...
-- backfill: هر رکورد قدیمی به اولین/پیش‌فرض‌ترین منبع همان tenant وصل می‌شود
...
-- psy_schedules قبلا unique(tenant_id, date) بود (یک برنامه‌ی مشترک برای کل
-- tenant)؛ حالا هر دکتر برنامه‌ی مستقل خودش را دارد.
...
-- ── 3) پروفایل per-resource (بج/نوع جلسه/کارت) ─────────────────────────────
...
-- backfill: پروفایل منبع پیش‌فرض هر tenant را از psy_clinic_settings قدیمی پر کن
...
begin
  if exists (select 1 from information_schema.columns where table_name = 'psy_clinic_settings' and column_name = 'badges') then
    ...
    -- نام/عنوان/آواتار دکتر قدیمی روی خود resources منتقل شود (این ستون‌ها از قبل آنجا هستند)
...
-- برای هر resource باقی‌مانده که هنوز پروفایل ندارد (مثلا tenant تازه‌ساز)، ردیف پیش‌فرض بساز
