-- ─────────────────────────────────────────────────────────────────────────────
-- سیستم حسابداری نوبت‌لینک (جولای ۲۰۲۶)
--
-- دو جدول:
-- 1) ledger_entries — دفتر حساب. هر تراکنش نهایی‌شده (آنلاین یا کارت‌به‌کارت)
--    یک ردیف *تغییرناپذیر* اینجا می‌سازد. منبع حقیقت حسابداری. هیچ‌وقت
--    ویرایش/حذف نمی‌شود؛ فقط INSERT. برای مستندات دائمی و ردگیری اختلاف.
-- 2) settlements — دفتر تسویه. وقتی سوپرادمین سهم یک دکتر را دستی/بانکی واریز
--    کرد، اینجا ثبت می‌کند تا معلوم باشد چه مقدار از بدهی پلتفرم به دکتر
...
create table if not exists ledger_entries (
  ...
  -- سهم پلتفرم و دکتر (فقط برای تراکنش آنلاین معنی دارد؛ کارت‌به‌کارت مستقیم
  -- بین مراجع و دکتر است، سهم پلتفرم = 0)
  commission_amount bigint not null default 0,
  doctor_amount bigint not null default 0,
  -- رد منبع — کدام رکورد این تراکنش را ساخت (برای حسابرسی/تطبیق)
  ...
  -- برای تراکنش آنلاین: آیا سهم دکتر خودکار (تسهیم زیبال) واریز شده؟
  split_applied boolean not null default false,
  -- چه کسی ثبت کرد (برای کارت‌به‌کارت = تاییدکننده)
...
-- جلوگیری از ثبت دوباره‌ی یک تراکنش واحد (idempotency) — یک source یک بار
...
create table if not exists settlements (
  ...
  tenant_id uuid not null references tenants(id) on delete cascade,
  resource_id uuid references resources(id),
  -- مبلغی که پلتفرم بابت سهم دکتر به او واریز کرده
  amount bigint not null,
  method text not null default 'manual',   -- 'manual' | 'auto_split'
  -- بازه‌ای که این تسویه پوششش می‌دهد (اختیاری، برای مستندسازی)
  covers_from timestamptz,
  covers_to timestamptz,
  reference text,                 -- شماره‌ی پیگیری واریز بانکی
  note text,
