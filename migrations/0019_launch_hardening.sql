-- ─────────────────────────────────────────────────────────────────────────────
-- سخت‌سازی پیش از لانچ — additive، امن روی دیتابیس زنده (هیچ drop/تغییری ندارد).
--
-- ⚠️ از این migration به بعد، قرارداد «بازسازی مخرب» (drop همه‌ی جدول‌ها) رسما
-- منسوخ است — با آمدن اولین مشتری واقعی، همه‌ی تغییرات اسکیما فقط additive اند.
--
-- ۱) ضامن همزمانی اسلات‌های روانشناسی در سطح دیتابیس:
--    تا امروز فقط جدول bookings (نیچ عمومی) unique index داشت؛ psy_stages و
--    psy_sessions فقط check-then-update در اپ داشتند که در برابر دو درخواست
--    هم‌زمان (دو مراجع روی یک ساعت) بی‌دفاع بود. این ایندکس‌ها partial اند تا
--    ردیف‌های بدون زمان (session_date خالی/null — جلسات هنوز زمان‌بندی‌نشده)
--    مشمول نشوند.
--    نکته: تداخل «بین» stage و session (دو جدول جدا) همچنان با چک اپ گرفته
--    می‌شود؛ این ایندکس‌ها ضامن نهایی هم‌زمانی «درون» هر جدول اند.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- ۲) یادآوری پیامکی خودکار — ردیابی «برای این نوبت یادآوری ارسال شده»:
