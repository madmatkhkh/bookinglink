-- ─────────────────────────────────────────────────────────────────────────────
-- additive — امن روی دیتابیس زنده. پیاده‌سازی درخواست «قابلیت‌هایی که رقبا
-- دارن و ما نداریم» (بجز همگام‌سازی گوگل‌کلندر و صورتحساب بیمه که کنار گذاشته شد).
-- ─────────────────────────────────────────────────────────────────────────────

-- ۱) ورود ایمیلی — برای مراجع/مشتری خارج از ایران که پیامک ایرانی بهش نمی‌رسه.
--    ستون phone در otps از این پس یک شناسه‌ی عمومی است (شماره یا ایمیل)؛
...
-- ۲) لینک گوگل‌میت جلسه‌ی آنلاین — بدون نیاز به OAuth/اتصال گوگل‌کلندر:
--    دکتر لینک ثابت خودش را (که از حساب Gmail/Google خودش ساخته) این‌جا
...
-- ۳) لیست انتظار — وقتی مراجع روزی را می‌خواهد که پر است
create table if not exists psy_waitlist (
  ...
  note text not null default '', -- مثلا «فقط عصرها» یا «تا آخر ماه»
...
-- ۴) نظر/امتیاز واقعی مراجع — جایگزین بج فیکس حذف‌شده
...
-- ۵) یادداشت بالینی ساختاریافته (SOAP/DAP) — کاملا خصوصی، هرگز به مراجع نشان
--    داده نمی‌شود؛ جدا از psy_sessions.doctor_note_for_patient (که برای مراجع است).
...
-- ۶) تاریخچه‌ی کمپین پیامکی/ایمیلی (بلاست به مراجعان) — فقط لاگ، ارسال هم‌زمان است
