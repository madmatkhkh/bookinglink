// ─────────────────────────────────────────────────────────────────────────────
// ارسالِ ایمیل — برایِ مراجع/متخصصی که خارج از ایران است و پیامکِ ایرانی به او
// نمی‌رسد. از Resend استفاده می‌کند (HTTP API ساده، بدونِ SMTP، مناسبِ سرورلس).
//
...
// OTP_ECHO_CODE=true قابلِ تست است.
// ─────────────────────────────────────────────────────────────────────────────
...
async function send(to: string, subject: string, html: string): Promise<EmailSendResult> {
  ...
  try {
    ...
    return { ok: false, error: data?.message || 'ارسالِ ایمیل ناموفق بود' }
  } catch (err) {
    console.error('resend network error:', err)
    return { ok: false, error: 'اتصال به سرویسِ ایمیل برقرار نشد' }
  }
}

/** کدِ ورودِ یک‌بارمصرف از طریقِ ایمیل — جایگزینِ پیامک برایِ مراجعِ خارج از ایران */
export async function sendOtpEmail(email: string, code: string): Promise<EmailSendResult> {
  return send(email, 'کدِ ورودِ شما', `
...
/** یادآوریِ نوبتِ فردا از طریقِ ایمیل — همتای sendReminderSms، با لینکِ گوگل‌میتِ
 * اختیاری برایِ جلساتِ آنلاین. */
export async function sendReminderEmail(
  email: string, name: string, date: string, time: string, meetLink?: string
): Promise<EmailSendResult> {
  return send(email, `یادآوریِ نوبتِ فردا — ${name}`, `
    <div dir="rtl" style="font-family: Tahoma, Arial, sans-serif; text-align: right;">
      <p>یادآوری: نوبتِ شما در <b>${name}</b> فردا <b>${date}</b> ساعتِ <b>${time}</b> است.</p>
      ${meetLink ? `<p>لینکِ جلسه‌ی آنلاین: <a href="${meetLink}">${meetLink}</a></p>` : ''}
    </div>`)
}

/** پیامِ آزادِ کمپین (بلاست) — برایِ اعلامیه‌ها/بازاریابیِ ساده به لیستِ مراجعان */
