...
// نقطه عمدا مجاز نیست: نشانی‌ای مثل dr.ali در پیامک و تلگرام شبیه دامنه دیده
// می‌شود و کلاینت‌ها اشتباه لینکش می‌کنند.
// شروع و پایان همیشه حرف یا عدد است تا نشانی‌هایی مثل `_a` یا `x-` نسازیم، و دو
// جداکننده‌ی پشت‌سرهم (`a--b`, `a-_b`) هم مجاز نیست چون در عمل تایپی هستند و
// نشانی‌های نزدیک‌به‌هم و گیج‌کننده می‌سازند.
// سقف طول (40) عمدا در متن خطا گفته نمی‌شود — هیچ‌کس نشانی چهل‌نویسه‌ای نمی‌سازد
// و گفتنش فقط پیام را شلوغ می‌کرد؛ محدودیت سر جایش هست.
...
export const SLUG_RULE_TEXT = 'حروف کوچک انگلیسی، عدد، خط تیره و زیرخط'
