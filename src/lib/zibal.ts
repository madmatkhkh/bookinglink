...
//
// ⚠️ کارمزد پلتفرم + تسهیم خودکار (جولای 2026): چون همه‌ی تراکنش‌های آنلاین از
// یک ZIBAL_MERCHANT_ID واحد (سراسری پلتفرم) رد می‌شوند، 100٪ پول همین الان
...
// قبل از روشن‌کردنش روی پروداکشن: (1) با ZIBAL_SANDBOX=true یک تراکنش تستی با
// یک شبای واقعی بزن، (2) گزارش تسهیم را در پنل زیبال چک کن که سهم درست تقسیم
// شده، (3) اگر زیبال «ذی‌نفع تعریف‌نشده» برگرداند، طبق مستندات زیبال باید اول
...
// ⚠️ فیکس مهم (جولای 2026): قبلا این‌جا از env var مستقل PLATFORM_COMMISSION_PERCENT
...
export async function requestZibalPayment(
  ...
  try {
    ...
    // عادی (100٪ به مرچنت پلتفرم، رفتار فعلی) بدون تغییر انجام می‌شود.
