...
//
// ⚠️ کارمزدِ پلتفرم + تسهیمِ خودکار (جولای ۲۰۲۶): چون همه‌ی تراکنش‌هایِ آنلاین از
// یک ZIBAL_MERCHANT_ID واحد (سراسریِ پلتفرم) رد می‌شوند، ۱۰۰٪ پول همین الان
// مستقیم به حسابِ صاحبِ پلتفرم می‌رود — نه هر tenant مرچنتِ خودش. یعنی «کارمزد
// گرفتن» به‌صورتِ خودکار همین الان دارد اتفاق می‌افتد؛ چیزی که کم دارد یک
// مکانیزمِ واریزِ سهمِ دکتر است. زیبال سرویسِ «تسهیم» دارد (تقسیمِ خودکارِ هر
// تراکنش بینِ چند شبا/ذی‌نفع، مستندات در help.zibal.ir/pdocs.zibal.ir) ولی چون
// آن صفحات برایِ fetchِ خودکار مسدودند، پیاده‌سازیِ زیر بر پایه‌ی یک کتابخانه‌ی
// غیررسمی (NuGet: ZibalClient) است، نه مستنداتِ رسمیِ تاییدشده. به همین دلیل
// پشتِ ZIBAL_MULTIPLEXING_ENABLED است و پیش‌فرض خاموش — حتی اگر شبایِ دکتر ثبت
// شده باشد، تا این env صریحاً true نشود، تراکنش عادی (بدونِ تسهیم) انجام می‌شود.
// قبل از روشن‌کردنش رویِ پروداکشن: (۱) با ZIBAL_SANDBOX=true یک تراکنشِ تستی با
// یک شبایِ واقعی بزن، (۲) گزارشِ تسهیم را در پنلِ زیبال چک کن که سهم درست تقسیم
// شده، (۳) اگر زیبال «ذی‌نفعِ تعریف‌نشده» برگرداند، طبقِ مستنداتِ زیبال باید اول
// از پنلِ کاربریِ خودِ زیبال (نه از این کد) هر شبا را به‌عنوانِ ذی‌نفع تعریف/تایید
// کنی. اگر فرمتِ درخواست را زیبال رد کرد، با پشتیبانیِ زیبال (تیکت) هماهنگ کن.
// ─────────────────────────────────────────────────────────────────────────────
...
// کارمزدِ پلتفرم روی هر تراکنشِ آنلاین — عددِ درصد، از env (بدونِ نیاز به تغییرِ
// کد برایِ عوض‌کردنش). مقدارِ پیش‌فرض فقط یک نقطه‌ی شروعِ موقت است.
export const PLATFORM_COMMISSION_PERCENT = Number(process.env.PLATFORM_COMMISSION_PERCENT) || 0

// آیا تلاش برایِ تسهیمِ خودکار (واریزِ مستقیمِ سهمِ دکتر به شبایِ خودش) فعال است؟
...
export async function requestZibalPayment(
  ...
  try {
    ...
    // تسهیم فقط اگر صریحاً روشن باشد و اطلاعاتِ شبا موجود باشد — وگرنه تراکنشِ
    // عادی (۱۰۰٪ به مرچنتِ پلتفرم، رفتارِ فعلی) بدونِ تغییر انجام می‌شود.
    if (MULTIPLEXING_ENABLED && split && split.doctorAmountToman > 0) {
      body.percentMode = 0 // 0 = مبلغِ ثابت (تومان/ریال)، نه درصد — سهمِ دکتر را خودمان از قبل محاسبه کرده‌ایم
      body.feeMode = 0 // کارمزدِ زیبال از خودِ تراکنش کسر می‌شود (سهمِ مرچنتِ اصلی، نه سهمِ دکتر)
