// ─────────────────────────────────────────────────────────────────────────────
// لایه‌ی حسابداری — نقطه‌ی واحدِ ثبت در دفترِ حساب (ledger_entries).
//
// اصلِ طراحی: هر جایی که یک تراکنش «نهایی» می‌شود (پرداختِ آنلاین در callbackِ زیبال،
// یا تاییدِ دستیِ کارت‌به‌کارت توسطِ دکتر) دقیقاً یک بار recordLedgerEntry صدا زده
// می‌شود. ثبت idempotent است: اگر همان (source_table, source_id, purpose) قبلاً
// ثبت شده باشد، دوباره ثبت نمی‌شود (unique index در دیتابیس هم پشتیبان است) — تا
// رفرشِ کاربر یا دابل‌کلیکِ دکتر باعثِ ردیفِ تکراری/جمعِ اشتباه نشود.
//
// دفتر تغییرناپذیر است: فقط INSERT، هیچ‌وقت UPDATE/DELETE. اگر تراکنشی برگشت خورد
// (بازپرداخت)، یک ردیفِ جدید با direction='outflow' ثبت می‌شود، نه ویرایشِ ردیفِ قبلی.
// ─────────────────────────────────────────────────────────────────────────────
import { sb } from './supabase'

export type LedgerPurpose = 'interview' | 'assessment' | 'package' | 'session' | 'refund'
...
// ثبتِ یک ردیفِ دفترِ حساب. برمی‌گرداند آیا ردیفِ تازه‌ای ثبت شد یا نه (false =
// از قبل بود، idempotent). هرگز throw نمی‌کند — خطایِ ثبتِ ledger نباید فلوِ
// اصلیِ پرداخت را بشکند؛ فقط لاگ می‌شود (ثبتِ بعدیِ تطبیقی می‌تواند جبرانش کند).
export async function recordLedgerEntry(input: RecordLedgerInput): Promise<boolean> {
  try {
    // چکِ idempotency در سطحِ اپ (unique index هم در DB هست، این فقط برایِ صرفه‌جویی)
