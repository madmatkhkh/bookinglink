...
// لغو رزرو توسط مراجع — فاز 1: فقط رزروهای آینده، بدون سازوکار بازپرداخت
export async function POST(req: NextRequest, { params }: { params: { slug: string } }) {
  ...
  if (!['pending_payment', 'payment_submitted', 'confirmed'].includes(b.status))
    return NextResponse.json({ error: 'این رزرو قابل لغو نیست' }, { status: 400 })
  if (b.booking_ts <= Date.now())
    return NextResponse.json({ error: 'زمان این نوبت گذشته است' }, { status: 400 })
