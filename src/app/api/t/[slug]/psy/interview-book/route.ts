...
export async function POST(req: NextRequest, { params }: { params: { slug: string } }) {
  ...
  const [{ data: takenS }, { data: takenB }, { data: takenI }, { data: takenA }] = await Promise.all([
    db.from('psy_sessions').select('id').eq('tenant_id', t.id).eq('session_date', interview_date).eq('session_time', interview_time),
    db.from('psy_cases').select('id, status').eq('tenant_id', t.id).eq('booking_date', interview_date).eq('booking_time', interview_time),
    db.from('psy_cases').select('id').eq('tenant_id', t.id).eq('interview_date', interview_date).eq('interview_time', interview_time).neq('id', booking.id),
    db.from('psy_cases').select('id').eq('tenant_id', t.id).eq('assessment_date', interview_date).eq('assessment_time', interview_time),
  ])
