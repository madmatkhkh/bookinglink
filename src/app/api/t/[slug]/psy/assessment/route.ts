...
export async function POST(req: NextRequest, { params }: { params: { slug: string } }) {
  ...
  const [{ data: takenS }, { data: takenB }, { data: takenA }] = await Promise.all([
    db.from('psy_sessions').select('id').eq('tenant_id', t.id).eq('resource_id', booking.resource_id).eq('session_date', assessment_date).eq('session_time', assessment_time),
    db.from('psy_cases').select('id, status').eq('tenant_id', t.id).eq('resource_id', booking.resource_id).eq('booking_date', assessment_date).eq('booking_time', assessment_time),
    db.from('psy_cases').select('id').eq('tenant_id', t.id).eq('resource_id', booking.resource_id).eq('assessment_date', assessment_date).eq('assessment_time', assessment_time),
  ])
