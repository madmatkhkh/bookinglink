...
// تغییرِ شماره‌ی ورودِ خودِ owner یا خودِ درمانگر (نه شماره‌ی هیچ‌کسِ دیگر) — دو قدم:
//   قدمِ ۱ (بدونِ code): اعتبارسنجی + چک‌کردنِ اینکه این شماره قبلاً مالِ یک
//           owner/درمانگرِ دیگر نباشد (تا دو نفر با هم قاطی نشوند) + صدورِ OTP.
//   قدمِ ۲ (با code): تاییدِ OTP → همان‌جا شماره در دیتابیس عوض می‌شود.
export async function POST(req: NextRequest, { params }: { params: { slug: string } }) {
  ...
  if (!b.code) {
    ...
    if (!issued.ok) {
      ...
      return NextResponse.json({ error: issued.smsError || 'ارسالِ پیامک ناموفق بود' }, { status: 502 })
    }
  ...
  if (result !== 'ok') return NextResponse.json({ error: 'کدِ واردشده درست نیست یا منقضی شده' }, { status: 400 })
